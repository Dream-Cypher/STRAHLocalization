...
- Prereqs: Python 3.10+. A plain text build (`convert_csv_to_json.py`) is **pure stdlib — nothing to install**. Non-stdlib deps go in a **project venv** (`.venv/`, gitignored); don't install into system Python. `requirements.txt` = `UnityPy` + `Pillow`, needed only to regenerate the image assets (`make_overlays.py` / `make_sprites.py`): `py -3.10 -m venv .venv` then `.venv\Scripts\python -m pip install -r requirements.txt`. (The translation/QA tooling and its `openpyxl` dep live in `local-scripts/` — see below.) .NET 8 SDK only if recompiling the helper; Windows x64 to RUN the helper.

## Pipeline scripts (scripts/ — what the repo ships, build only)
The repo keeps ONLY the scripts needed to build the patch. The translation
grunt-work and QA tooling was moved to `local-scripts/` (gitignored, see below).
- `convert_csv_to_json.py` — CSV→JSON, the build entry. Wraps XMESS dialogue at `DIALOGUE_WIDTH` (48), re-flow, word-breaks only, `<color>` tags zero-width. Translates tips/flowchart/flymove/metadata/choices. Does NOT translate the speaker `name` field (see Gotchas). Pure stdlib. (Optionally reads the gitignored `texts/en/_speaker_names.json` if present — wrapped in try/except, so its absence is fine.)
- `make_overlays.py` / `make_sprites.py` — regenerate the image-form text assets (name plates, QTE prompts, manual) into `files/sprites/` & `files/images/`. Need `UnityPy` + `Pillow` (`requirements.txt`). `make_overlays.py --only <sprite>` regenerates one plate; the `--only` flag takes a SINGLE value (run it once per sprite).
- `download_fonts*.ps1` — optional font staging (Inter = Western opt-in; MiSans = original Chinese). See BUILD_ENGLISH_PATCH.txt.

## local-scripts/ (gitignored — translation + QA tooling, NOT in the repo)
Frequently-changing grunt work, kept local-only. Deps: `local-scripts/requirements.txt` (`openpyxl`); LLM translators use stdlib `urllib` → Ollama. Run from the repo root (they resolve paths via `dirname(dirname(__file__))`, same depth as `scripts/`).
- `translate_files.py` — Ollama (`qwen3:30b-a3b`; host `http://$OLLAMA_IP:11434`, default IP `10.219.72.133`, LAN-only — override with the `OLLAMA_IP` env var) per-file translator: gap-fill, runaway guard, keep-first dedup. `is_untranslated()` flags any kana/kanji. `translate_local.py`, `consolidate_chunks.py`, `apply_translations.py` round out the LLM path.
- Cleanup passes: `fill_untranslated.py`, `condense_long.py` (both `import translate_files`), `repair_speaker_echo.py`, `normalize_en.py`, `fix_terms.py`, `strip_markers.py`. `build_speaker_names.py` regenerates the gitignored `texts/en/_speaker_names.json`.
- **Translation review (play-through QA)** — needs `openpyxl`:
  ...
  - `audit_gender.py` / `audit_translations.py` → `audit_gender` flags he/she pronoun errors by comparing EN against the gendered Chinese (他/她) / Japanese (彼/彼女) reference (catches MT mistakes where JA omitted the pronoun). Conservative; seeds `GENDER(...)` tags into the `notes` column + a report with ±2 surrounding lines for context. `--prefill` seeds case-preserving he→she swaps into `proposed_fix` for high-conf rows. (2026-06 pass: 65 lines corrected; 7 ambiguous keeps remain, intentionally.)
